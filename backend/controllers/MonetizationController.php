        ...
        echo json_encode([
            ...
            'config' => [
                ...
                'commission_min' => (float)$config['payment_min_commission'],
                 ...
                 'featured_cost' => (float)($config['featured_cost'] ?? 5.00),
                 'featured_duration_days' => (int)($config['featured_duration_days'] ?? 7),
...
public function updateConfig(): void
{
    ...
    $stmt = $this->conn->prepare("
        UPDATE system_config SET
            ...
            featured_duration_days = :featured_duration
        WHERE id = 1
    ");
    ...
    $stmt->execute([
        ...
        ':featured_cost' => $input['featured_cost'] ?? 5.00,
        ':featured_duration' => $input['featured_duration_days'] ?? 7
    ]);
...
/**
 * Renovar publicación de servicio
 * POST /api/monetization/renew-publish
 */
public function renewPublish(): void
{
    $this->requireAuth();
    $input = json_decode(file_get_contents('php://input'), true);
    ...
    $config = $this->getSystemConfig();
    $amount = (float)$config['service_publish_cost'];
    $duration = (int)$config['service_publish_duration'];

    if ($amount <= 0) {
        http_response_code(400);
        echo json_encode(['error' => 'La renovación es gratuita']);
        return;
    }
    ...
    // Guardar comprobante
    $stmt = $this->conn->prepare("
        INSERT INTO service_payment_proofs (service_id, provider_id, amount, payment_method, reference, status)
        VALUES (?, ?, ?, ?, ?, 'pending')
    ");
    $stmt->execute([$serviceId, $this->user->id, $amount, $method, $reference]);

    // Extender fecha si se aprueba (el admin lo hará manualmente)
    echo json_encode([
        'success' => true,
        'message' => "Comprobante de renovación enviado. El administrador verificará tu pago y extenderá la publicación por {$duration} días."
    ]);
}

/**
 * Verificar servicios próximos a vencer (para notificar)
 * GET /api/admin/expiring-services
 */
public function getExpiringServices(): void
{
    $this->requireAdmin();

    $stmt = $this->conn->query("
        SELECT s.id, s.title, s.expires_at, u.name as provider_name, u.email
        FROM services s
...
/**
 * Pagar para destacar un servicio
 * POST /api/monetization/feature
 */
...
if ($existingPayments > 0) {
   ...
   // 5. Validar límite de destacados por usuario (máximo 3)
    ...
    $duration = (int)($config['featured_duration_days'] ?? 7);
    $method = $input['payment_method'] ?? 'transferencia';
    $reference = $input['reference'] ?? '';

    // Guardar comprobante
    $stmt = $this->conn->prepare("
        INSERT INTO service_payment_proofs (service_id, provider_id, amount, payment_method, reference, proof_url, status, payment_type)
    ...
    echo json_encode([
        'success' => true,
        'message' => "Comprobante de destacado enviado. El administrador verificará tu pago y destacará tu servicio por {$duration} días."
    ]);
}

/**
 * Obtener costo de destacado (público)
 * GET /api/monetization/feature-cost
 */
public function getFeatureCost(): void
{
    $config = $this->getSystemConfig();
    echo json_encode([
        'success' => true,
