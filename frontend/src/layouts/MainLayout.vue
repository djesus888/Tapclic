<template>
  <div class="app-container bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen flex flex-col">
    ...
    <NotificationModal
      ...
      @open-payment-proof="handleOpenPaymentProof"
    />

    <!-- ========== MODAL DE DETALLE DE DISPUTA ========== -->
    <DisputeDetailModal
      :is-open="showDisputeModal"
      :dispute-id="selectedDisputeId"
      @close="showDisputeModal = false; selectedDisputeId = null"
      @updated="notificationStore.loadNotificationsFromAPI()"
    />

    <!-- 🔥 NUEVO: MODAL DE VERIFICACIÓN DE PAGO -->
    <ProofModal
      v-if="showPaymentProofModal"
      :request-id="selectedPaymentRequestId"
      @close="showPaymentProofModal = false; selectedPaymentRequestId = null"
    />
  </div>
</template>
...
import DisputeDetailModal from '@/components/DisputeDetailModal.vue'
import ContentBlock from '@/components/shared/ContentBlock.vue'
// 🔥 NUEVO: Importar ProofModal
...
// 🔥 NUEVO: Estado para el modal de verificación de pago
  ...
  try {
    ...
    if (notification.data_json) {
      try {
        ...
        // 🔥 CORRECCIÓN: Abrir modal de verificación de pago para payment_received
        if (data.notification_type === 'payment_received' || data.type === 'payment' || data.action === 'verify_payment') {
          selectedPaymentRequestId.value = data.request_id || notification.request_id || null
          if (selectedPaymentRequestId.value) {
...
const handleOpenPaymentProof = (data) => {
  if (data?.request_id) {
    selectedPaymentRequestId.value = data.request_id
    showPaymentProofModal.value = true
    selectedNotification.value = null
  }
}
