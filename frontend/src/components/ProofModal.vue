    ...
    <div class="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 space-y-4">
      ...
      >
        <div class="grid grid-cols-2 gap-2 text-sm">
          ...
          <div v-if="proof.reference">
            <strong>Referencia:</strong> {{ proof.reference }}
          </div>
          <div class="col-span-2">
            <strong>Estado actual:</strong> <PaymentPill :status="proof.status" />
          </div>
        </div>

        <!-- IMAGEN -->
        <div
          v-if="proof.proof_url"
          class="mt-4"
        >
          <img
