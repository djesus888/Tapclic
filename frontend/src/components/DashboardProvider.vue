...
export default {
  ...
  components: { ChatRoomModal, NewTicketModal, PaymentPill, ProofModal, SolicitudesDisponibles, SolicitudesActivas, Soporte, Historial, RequestConfirmationModal, ProviderContactModal, PaymentModal, LiveOrderTracking, ReviewModal },
  setup() {
  ...
  async mounted() {
    ...
    try {
      ...
      // ✅ Configurar callbacks para que el composable pueda refrescar datos
      this.setRefreshCallbacks({
        activeRequests: () => this.fetchActiveRequests(),
        history: () => this.fetchHistoryRequests(),
        services: () => {} // No aplica para provider
      });
      // ✅ Iniciar listeners de socket como cliente
      this.setupClientSocketHandlers();
  ...
  beforeUnmount() {
    ...
    // ✅ Limpiar también los handlers de cliente
    this.cleanupClientSocketHandlers();
    this.resetModals();
