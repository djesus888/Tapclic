        ...
        >
          <div class="p-4 flex justify-between items-start bg-gray-100">
            ...
            <div class="text-right ml-4">
              ...
              <span
                :class="service.status === 'available' ? 'text-green-600' : 'text-red-600'"
          ...
          <div class="p-4 flex justify-between items-center bg-white">
            <div class="flex items-center gap-3">
              <img
                :src="service.provider?.avatar_url || '/img/default-provider.png'"
                :alt="service.provider?.name || 'Desconocido'"
                class="w-10 h-10 rounded-full object
cover"
              />
              <div>
    ...
    <div v-if="selectedTab === 'activeRequests'">
      ...
      <div v-else>
        <div v-if="activeRequests.length === 0" class="text-center text-gray-500 py-10">
          {{ $t('no_active_requests') }}
        </div>
        <div class="grid gap-4 md:grid-co ls-2 lg:grid-cols-3">
          <div
          ...
          >
            <div class="p-4 flex justify-between items-start bg-gray-100">
              ...
              <div class="text-right ml-4">
                ...
                <span
                  :class="request.status === 'accepted' ? 'text-green-600' : 'text-yellow-600'"
                  class="text-xs font-medium"
                >
            ...
            <div class="p-4 flex justify-between items-center bg-white">
              <div class="flex items-center gap-3">
              <img
               :src="request.service_image || '/img/default-provider.png'"
 ...
 <ChatRoomModal
  ...
  />
    ...
    <ProviderContactModal
      ...
      @on-open-change="val => showProviderContact = val"
      @openPayment="() => { showProviderContact = false; showPayment = true }"
...
export default {
  ...
  try {
    const authStore = useAuthStore();
    const res = await api.post(this.buildPath('requests/active'), {}, {
      headers: authStore?.token ? { Authorization: `Bearer ${authStore.token}` } : {}
    });
    ...
    const requests = Array.isArray(res.data)
      ...
      : Array.isArray(res.data?.requests)
        ? res.data.requests
        : Array.isArray(res.data?.data) // algunos backends devuelven así
          ? res.data.data
          : [];
    ...
    async onConfirmRequest(specDetails) {
      try {
        ...
        const payload = {
          ...
          payment_method: this.selectedPaymentMethod || 'efectivo',
        ...
        const initialStatus = res.data.status;

        this.modalService.requestId = requestId;
        ...
        this.$nextTick(() => {
          ...
          if (providerModal) {
            providerModal.status = initialStatus;
