    ...
    <template v-else>
      ...
      <div class="header">
        <div class="title-section">
          <h1><span class="icon">{{ isFeature ? '⭐' : '💳' }}</span> {{ isFeature ? 'Destacar Servicio' : 'Publicar Servicio' }}</h1>
          <p>{{ isFeature ? 'Completa el pago para destacar tu servicio' : 'Completa el pago para activar tu servicio' }}</p>
        </div>
        <router-link to="/myservices" class="btn-back-link">← Volver</router-link>
      </div>
        ...
        <div class="summary-card">
          ...
          <div class="summary-details">
            <div class="detail-row highlight">
              <span>{{ isFeature ? 'Costo de destacado' : 'Costo de publicación' }}</span>
      ...
      <div v-if="payMethod !== 'wallet' && selectedMethod" class="transfer-section">
        ...
        <div class="upload-section">
          ...
          <div class="form-group">
            ...
            <input v-model="reference" type="text" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" placeholder="Ej: 123456" class="form-input" required title="Debe ser exactamente 6 dígitos numéricos" />
          </div>
        </div>
      </div>
      ...
      <div v-if="publishCost === 0" class="free-publish">
        <p>🎉 {{ isFeature ? 'El destacado' : 'Este servicio' }} es de <strong>publicación gratuita</strong></p>
        <button class="btn-pay" @click="activateFree" :disabled="paying">
          {{ paying ? '⏳ Activando...' : isFeature ? '⭐ Destacar gratis' : '🚀 Publicar gratis' }}
        </button>
      </div>
    </template>
...
// ✅ Leer tipo de pago: publish o feature
const paymentType = ref(route.query.type || 'publish')
const isFeature = computed(() => paymentType.value === 'feature')
...
const loadServiceData = async () => {
  ...
  try {
    ...
    const costEndpoint = isFeature.value ? '/monetization/feature-cost' : '/monetization/publish-cost'
    ...
    // ✅ Leer costo según tipo
    publishCost.value = parseFloat(costRes.data?.featured_cost || costRes.data?.publish_cost || 0)
...
const activateFree = async () => {
  ...
  paying.value = true
  try {
    // ✅ Usar endpoint correcto según tipo
    const endpoint = isFeature.value 
      ? `/monetization/feature` 
      : `/services/${serviceId}/publish`
    
    const payload = isFeature.value ? { service_id: serviceId } : {}
    
    const { data } = await api.post(endpoint, payload, {
      headers: { Authorization: `Bearer ${authStore.token}` }
    })
    showToast(data?.message || (isFeature.value ? 'Servicio destacado gratuitamente' : 'Servicio publicado gratuitamente'))
...
const processPayment = async () => {
  ...
  const endpoint = isFeature.value
    ? '/monetization/feature'
    : `/services/${serviceId}/publish-external`
