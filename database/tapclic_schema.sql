...
CREATE TABLE `dispute_messages` (
  ...
  `sender_role` enum('user','provider','admin') NOT NULL,
...
) ENGINE=InnoDB AUTO_INCREMENT=400 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `provider_billing` (
  ...
  `total_transactions` int(11) DEFAULT 0,
  `status` enum('pending','reported','paid','overdue','verifying','cancelled') DEFAULT 'pending',
...
CREATE TABLE `services` (
  ...
  `notified_featured_expiry` tinyint(1) DEFAULT 0,
...
CREATE TABLE `system_config` (
  ...
  `strong_passwords` tinyint(1) DEFAULT 1,
  `multiple_sessions` tinyint(1) DEFAULT 1,
  ...
  `featured_cost` decimal(10,2) DEFAULT 5.00,
  `featured_duration_days` int(11) DEFAULT 7,
  `monetization_model` enum('commission','publish','both','featured') DEFAULT 'commission',
  ...
  `max_featured_services` int(11) DEFAULT 10,
  PRIMARY KEY (`id`)
