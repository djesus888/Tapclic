...
// ============================================================
// Rate Limiting para endpoints de emisión
// ============================================================
...
app.use('/broadcast-all', emitLimiter);
...
io.on('connection', (socket) => {
    ...
    socket.device_id = socket.handshake.auth?.device_id || socket.handshake.query?.device_id || null;
    const userKey = buildUserKey(id, role);
    const room = `${role}_${id}`;

    // Manejo mejorado de conexiones duplicadas
    if (userSockets.has(userKey)) {
        const oldSocket = userSockets.get(userKey);

        if (oldSocket && oldSocket.connected) {
            console.log(`⚠️ Conexión duplicada para ${userKey}, cerrando conexión vieja (${oldSocket.id})`);

            // 🔥 CORRECCIÓN: Emitir evento con información del device
            oldSocket.emit('duplicate_connection', {
                message: 'Nueva conexión detectada, cerrando esta sesión',
                newSocketId: socket.id,
                device_id: oldSocket.device_id,
                new_device_id: socket.device_id,
            ...
            setTimeout(() => {
                if (userSockets.get(userKey) === oldSocket) {
                    userSockets.delete(userKey);
                    console.log(`🗑️ Socket viejo eliminado de userSockets: ${userKey}`);
                }
            }, 100);
        } else {
            userSockets.delete(userKey);
            console.log(`🗑️ Socket muerto eliminado de userSockets: ${userKey}`);
        }
    }
    ...
    socket.on('join-room', (roomName, callback) => {
        try {
            if (!roomName.startsWith('conversation_') && !roomName.startsWith('user_')) {
    ...
    socket.on('disconnect', (reason) => {
        const wasReplaced = socket._replacedByNewConnection === true;
        ...
        if (!wasReplaced) {
            ...
            console.log(`♻️ Desconexión por reemplazo: ${userKey}`);
        }
...
app.post('/emit', (req, res) => {
    ...
    console.log("📥 [EMIT] BODY RECIBIDO:", JSON.stringify({
        ...
        conversation_id,
        device_id,
        new_device_id: req.body.new_device_id,
        hasPayload: !!payload,
    ...
    if (event) {
        ...
        roomsArray.forEach(room => {
            ...
            if (socketsInRoom && socketsInRoom.size > 0) {
                if (event === 'session_closed' && device_id) {
                    // 🔥 CORRECCIÓN: Filtrar correctamente por device_id
                    const sockets = Array.from(socketsInRoom);
                    const targetSockets = sockets.filter(socketId => {
                        const s = io.sockets.sockets.get(socketId);
                        if (!s) return false;
                        
                        // Si el socket tiene device_id igual al device_id del evento (viejo), INCLUIR
                        if (s.device_id && String(s.device_id) === String(device_id)) {
                            return true;
                        }
                        
                        // Si el socket tiene device_id igual al new_device_id (nuevo), EXCLUIR
                        if (req.body.new_device_id && s.device_id && String(s.device_id) === String(req.body.new_device_id)) {
                            return false;
                        }
                        
                        // Si no tiene device_id, incluir por defecto
                        return true;
                    });
                    ...
                    if (targetSockets.length > 0) {
                        ...
                        console.log(`📡 [EMIT] session_closed entregado a ${targetSockets.length} sockets (device_id: ${device_id})`);
                    } else {
                        console.log(`📡 [EMIT] session_closed no entregado: no hay sockets con device_id ${device_id}`);
                    }
                } else {
...
app.get('/status', (req, res) => {
    ...
    res.json({
        ...
        broadcast_all_users: broadcastRoomSize
    });
});
...
// ============================================================
// Limpieza periódica de eventos pendientes
// ============================================================
setInterval(() => {
    const now = Date.now();
    let cleanedCount = 0;
